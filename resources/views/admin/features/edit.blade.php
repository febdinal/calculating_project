...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <form method="POST" action="{{ route('admin.features.update', $feature) }}" class="p-6 rounded-2xl tally-card space-y-5">
        ...
        <div id="price_container">
            ...
            <input type="text" inputmode="numeric" name="price" id="price_input" value="{{ old('price') ? (is_numeric(old('price')) ? 'Rp ' . number_format(old('price'), 0, ',', '.') : old('price')) : 'Rp ' . number_format($feature->calculated_price, 0, ',', '.') }}" placeholder="Rp 0" class="input-rupiah w-full px-3.5 py-2.5 rounded-xl bg-[var(--tally-input-bg)] border border-[var(--tally-input-border)] text-[var(--tally-ink-0)] text-xs font-mono focus:outline-none focus:ring-1 focus:ring-indigo-500 shadow-xs">
            <p class="text-[11px] text-[var(--tally-ink-2)] mt-1 font-mono" id="price_help">Jika fitur memiliki sub-fitur di bawah, harga fitur utama adalah akumulasi total harga sub-fitur.</p>
        </div>
        ...
        <div id="sub_features_container" class="p-4 rounded-xl bg-[var(--tally-subtle-bg)] border border-[var(--tally-card-border)] space-y-3">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-[11px] text-[var(--tally-ink-2)]">Setiap sub-fitur memiliki harga tersendiri. Totalnya otomatis menjadi harga fitur utama.</p>
                </div>
            ...
            <div id="sub_features_list" class="space-y-2">
                @foreach ($feature->subFeatures as $index => $sub)
                    <div class="flex items-center gap-2 sub-row">
                        ...
                        <input type="text" name="sub_features[{{ $index }}][name]" value="{{ $sub->name }}" placeholder="Nama Sub Fitur" required class="flex-1 px-3 py-1.5 rounded-lg bg-[var(--tally-input-bg)] border border-[var(--tally-input-border)] text-[var(--tally-ink-0)] text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500 shadow-xs">
                        <div class="w-44">
                        ...
                        <input type="number" name="sub_features[{{ $index }}][sort_order]" value="{{ $sub->sort_order }}" placeholder="Urutan" class="w-16 px-2 py-1.5 rounded-lg bg-[var(--tally-input-bg)] border border-[var(--tally-input-border)] text-[var(--tally-ink-0)] text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500 font-mono shadow-xs">
...
<script>
    ...
    function addSubFeatureRow(name = '', price = '') {
        ...
        row.innerHTML = `
            ...
            <input type="text" name="sub_features[${nextSubIndex}][name]" value="${name}" placeholder="Nama Sub Fitur Baru" required class="flex-1 px-3 py-1.5 rounded-lg bg-[var(--tally-input-bg)] border border-[var(--tally-input-border)] text-[var(--tally-ink-0)] text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500 shadow-xs">
            <div class="w-44">
            ...
            <input type="number" name="sub_features[${nextSubIndex}][sort_order]" value="${nextSubIndex}" placeholder="Urutan" class="w-16 px-2 py-1.5 rounded-lg bg-[var(--tally-input-bg)] border border-[var(--tally-input-border)] text-[var(--tally-ink-0)] text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500 font-mono shadow-xs">
            <button type="button" onclick="this.closest('.sub-row').remove(); calculateTotalFromSub();" class="tally-btn px-2 py-1.5 rounded-lg bg-rose-50 dark:bg-rose-950/50 hover:bg-rose-100 dark:hover:bg-rose-900 text-rose-600 dark:text-rose-300 text-xs border border-rose-500/20 font-bold">
                ×
            </button>
        `;
